...
class detector:
    ...
    def __init__(self, kappa_nor, mu, img_rows, img_cols, level, rgb=False, max_mutation=5000, alpha=0.05, beta=0.05, sigma=0.01):
        self.alpha = alpha
        self.beta = beta
        self.sigma = sigma
    ...
    def detect(self, orig_img, orig_label, sess, x, preds, feed_dict=None):
        ...
        while (not stop):
            ...
            mt = MutationTest(self.img_rows, self.img_cols, 1, self.max_mutation, self.level)
            if self.rgb:
