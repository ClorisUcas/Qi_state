...
def load_characteristics(dataset, attack):
    ...
    file_name = os.path.join('../detection/bu/', "%s_%s.npy" % (dataset, attack))
...
def bu_detect(datasets, model_path, sample_path, store_path, attack_type):
...
def main(args):
    bu_detect(FLAGS.datasets, FLAGS.model_path, FLAGS.sample_path, FLAGS.store_path, FLAGS.attack_type)
