...
import nmutant_util.utils as utils
from nmutant_model.model import Model, CallableModelWrapper

_logger = utils.create_logger("nmutant_attack.attacks")
...
class Attack(object):
    ...
    def __init__(self, model, back='tf', sess=None):
        """
        :param model: An instance of the nmutant_model.model.Model class.
        ...
        if not isinstance(model, Model):
            raise ValueError("The model argument should be an instance of"
                             " the nmutant_model.model.Model class.")
...
class FastGradientMethod(Attack):
    ...
    def __init__(self, model, back='tf', sess=None):
        ...
        nmutant_model.model.Model abstraction provided by nmutant_model.
        """
        if not isinstance(model, Model):
...
class BasicIterativeMethod(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        for i in range(self.nb_iter):
            ...
            eta = adv_x - x
            from nmutant_util.utils_tf import clip_eta
...
class MomentumIterativeMethod(Attack):
    ...
    def generate(self, x, **kwargs):
        ...
        from nmutant_util import utils_tf
        for i in range(self.nb_iter):
            # Compute loss
...
class MadryEtAl(Attack):
    ...
    def attack_single_step(self, x, eta, y):
        ...
        """
        import tensorflow as tf
        from nmutant_util.utils_tf import model_loss, clip_eta
