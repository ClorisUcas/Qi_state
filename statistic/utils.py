...
def init_coverage_tables(model):
    ...
    for layer in model.layers:
        ...
        for index in range(layer.output_shape[-1]):
            model_layer_dict[(layer.name, index)] = 0
    return model_layer_dict

def neuron_covered(model_layer_dict):
    result = sorted(model_layer_dict.items(), key=lambda model_layer_dict:model_layer_dict[1],reverse=True)
    num = int(np.ceil(len(model_layer_dict)*0.1))
    total = 0
    count = 0
    for v in model_layer_dict.values():
        total += v
    for v in range(num):
        count += result[v][1]
    ...
    for i, intermediate_layer_output in enumerate(intermediate_layer_outputs):
        for j in range(len(intermediate_layer_output)):
            ...
            for num_neuron in xrange(len(intermediate_neuron_output)):
                if intermediate_neuron_output[num_neuron] > threshold :
                    model_layer_dict[(layer_names[i], num_neuron)] += 1
