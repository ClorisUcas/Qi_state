...
FLAGS = flags.FLAGS
# sc有两个Surprise adequency 一个是LSA算法（欧式距离），一个是DSA（KDE算法）算法
# 衡量的是新的数据和所有训练数据，计算两者在每一层的输出，计算欧式距离，距离最近的认为是最好的数据
def sc(datasets, model_name, samples_path, others='', test_num=0, train_num=0, layer=-3, num_section=1000, de=False, attack='fgsm', epoch=9,datasettype = 'test'):
    ...
    if de == True:
        ...
        store_path = "../surprise/" + datasets + "/" + model_name + "/" + attack + '/'
    else:
        train_samples = X_train
        store_path = "../surprise/" + datasets + "/" + model_name + "/ori/"
    # 记录训练数据在倒数第三层的输出
    if not os.path.exists(store_path):
        ...
        for num in range(n_batches):
            ...
            sess, preds, x, y, model, feed_dict = model_load(datasets=datasets, model_name=model_name, de=de,
                                                             attack=attack, epoch=epoch,others=others)
    ...
    if os.path.exists(kdes_store_path):
        ...
        for i in range(10):
            ...
            kdes[i] = KernelDensity(kernel='gaussian', bandwidth=scott_bw).fit(a_n_train[class_inds[i]])
...
if __name__ == '__main__':
    # cifar10_googlenet12_10000_10000_1000
    datasets = 'cifar10'
    model = 'googlenet12'
    epoch = 3
    factor = '10000_10000_1000'
    train_num = 10000
    ...
    # flags.DEFINE_string('model', 'googlenet16', 'The name of model')
    ...
    # flags.DEFINE_integer('epoch',4, 'epoch')
    tf.app.run()