...
def model_train_eval(sess, x, y, predictions, X_train, Y_train,  X_val, Y_val,X_test,Y_test,save=False,
    ...
    # tf.summary.image("input", image_shaped_input, max_outputs=args.batch_size)  # 记录输入数据
    # Define loss
    ...
    with tf.name_scope("accuracy"):
        ...
        # accuracy=model_eval(sess, x, y, predictions, X_test=X_train[40000:] , Y_test=Y_train[4000:],
        #            feed=None, args=None)
    ...
    #
    #     train_step = train_step.minimize(loss)
    train_writer = tf.summary.FileWriter(args.train_dir + "/logs/train", sess.graph)

    test_writer = tf.summary.FileWriter(args.train_dir+ "/logs/val/")
    print('save_Log', args.train_dir + "/logs/")
    # train_step = train_step.minimize(loss)
    ...
    with sess.as_default():
        ...
        for epoch in xrange(args.nb_epochs):
            ...
            for batch in range(nb_batches):
                ...
                train_summary, _, train_loss, train_acc = sess.run([merged, train_step, loss, accuracy],feed_dict=feed_dict)
                #train_writer.add_summary(train_summary, globa_step.eval(sess))
                train_losses.append(train_loss)
                train_accs.append(train_acc)
                train_loss = np.mean(train_losses)
                train_acc = np.mean(train_accs)
                train_manually_summary = tf.summary.Summary(
                    value=[tf.summary.Summary.Value(tag="loss", simple_value=train_loss)])
                train_writer.add_summary(train_manually_summary, epoch * 1000 + batch)
            ...
            if epoch % 1 == 0:
                ...
                test_acc,test_loss = model_eval(sess, x, y, predictions, X_test=X_test, Y_test=Y_test, feed=None,
                                      args=eval_params)
                val_acc,val_loss = model_eval(sess, x, y, predictions, X_test=X_val, Y_test=Y_val, feed=None,
                                       args=eval_params)
                ...
                train_manually_summary = tf.summary.Summary(
                    value=[tf.summary.Summary.Value(tag="train/loss", simple_value=val_loss),
                        tf.summary.Summary.Value(tag="train/acc", simple_value=val_acc)])
                train_writer.add_summary(train_manually_summary, epoch )
                test_manually_summary = tf.summary.Summary(value=[tf.summary.Summary.Value(tag="val/loss", simple_value=test_loss),
                    tf.summary.Summary.Value(tag="val/acc", simple_value=test_acc)])
                test_writer.add_summary(test_manually_summary, epoch )
               # print(args.train_dir)
                assert end >= len(X_train)  # Check that all examples were used
                if save or epoch == args.nb_epochs - 1: #and ((epoch + 1) % bp == 0 or epoch == args.nb_epochs - 1):
                    train_dir = os.path.join(args.train_dir, str(epoch))
                    if not os._exists(train_dir):
                        os.makedirs(train_dir)
                    print('save',os.path.join(train_dir, args.filename))
        ...
        # 加载日志数据
        event_train=os.listdir(args.train_dir + "/logs/train")
        ea = event_accumulator.EventAccumulator( args.train_dir + "/logs/train/"+event_train[0])
        ea.Reload()
        event_test = os.listdir(args.train_dir + "/logs/val")
        eb = event_accumulator.EventAccumulator(args.train_dir + "/logs/val/"+event_test[0])
        eb.Reload()
        # train_loss=ea.scalars.Items('train/loss')
        # print(len(train_loss))
        # print([(i.step,i.value) for i in train_loss])
        print(ea.scalars.Keys())
        print(eb.scalars.Keys())
        # train_loss=ea.scalars.Items('train/loss')
        # print(len(train_loss))
        # print([(i.step,i.value) for i in train_loss])
        val_acc = ea.scalars.Items('train/acc')
        val_loss = ea.scalars.Items('train/loss')
        test_acc = eb.scalars.Items('val/acc')
        test_loss = eb.scalars.Items('val/loss')

        fig = plt.figure(figsize=(10, 4))
        ax1 = fig.add_subplot(122)
        ax1.plot([i.step for i in val_acc], [i.value for i in val_acc], label='train/acc')
        ax1.set_xlim(0)
        ax1.plot([i.step for i in test_acc], [i.value for i in test_acc], label='test/acc')
        ax1.set_xlabel("epoch")
        ax1.set_ylabel("")
        plt.legend(loc='lower right')
        ax2 = fig.add_subplot(121)
        ax2.plot([i.step for i in val_loss], [i.value for i in val_loss], label='train/loss')
        ax2.set_xlim(0)
        ax2.plot([i.step for i in test_loss], [i.value for i in test_loss], label='test/loss')
        ax2.set_xlabel("epoch")
        ax2.set_ylabel("")
        plt.legend(loc='upper right')
        plt.savefig( args.train_dir + '/logs/result.jpg')
        #plt.show()
        # if save:
...
def model_train(sess, x, y, predictions, X_train, Y_train, save=False,
    ...
    test_writer = tf.summary.FileWriter(args.train_dir+ "/logs/test/")
    print('save_Log', args.train_dir + "/logs/")
    # train_step = train_step.minimize(loss)
